...
- 规则：标题、考试时长、总分数、合格线、合格分数、策略、模式列表。
  - 标题：考试规则的标题。 
...
erDiagram
  ...
  ExamPaper ||--o{ ExamQuestionGroup : OneToMany
  ExamQuestionGroup ||--o{ ExamQuestionAnswer : OneToMany
  ExamQuestionAnswer ||--|| ExamQuestion : OneToOne
  ExamQuestionBank {
    bigint    id
    varchar   title
    bigint    subject_id
...
- 题库：包含标题、科目、描述、试题列表。
  ...
  - 试题列表：试题包含编码、题型、难度、题干、选项列表、正确选项、答案、答案URL、解析、解析URL、备注。
    ...
    - 备注：对题干进行补充说明。
- 试卷：包含大题列表、作答开始时间、是否提交、提交时间、阅卷开始时间、是否完成、完成时间、结果、总得分。
  - 大题列表：大题包含作答列表、顺序、标题、题型、总分数、得分和
    - 作答列表：作答包含试题编码、单选选项、多选选项、是否正确、作答内容、作答链接、阅卷批注、试题得分。
      - 试题编码：即试卷包含的试题，通过编码一一对应。
      - 单选选项：即判断题或单选题的某一选项。
      - 多选选项：即多选题的选项列表。
      - 是否正确：对于客观题，通常在提交试卷时，自动判断答案是否正确。
      - 作答内容：代表对主观题的回答文字。
      - 作答链接：通常是对主观题的回答的图片链接。
