...
public class RunAsSystemAspect {
     ...
     * 创建时使用随机密码，避免被猜到系统用户。
     */
    private final Authentication system = Authentications.ofSystem();
    ...
    public Object runAsSystemAround(ProceedingJoinPoint point) throws Throwable {
        ...
        if (Optional.ofNullable(oldContext)
            ...
            if (log.isDebugEnabled()) {
                log.debug(I18nHolder.getAccessor().getMessage("RunAsSystemAspect.systemUserHandle.hasRunAsSystem",
        ...
        // 但 RunAsManager 是在已认证的情况下，为了调用更高权限的本地方法或远程调用方法，采取的折中措施
        ...
        if (log.isDebugEnabled()) {
            log.debug(I18nHolder.getAccessor().getMessage(
                    "RunAsSystemAspect.systemUserHandle.replace", new Object[]{oldContext},
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug(I18nHolder.getAccessor().getMessage(
                        "RunAsSystemAspect.systemUserHandle.restore", new Object[]{oldContext},
