package com.github.mrzhqiang.rowing.basic.data;
...
public class DataDictServiceJpaImpl implements DataDictService {
    ...
    private void handleEnum(Class<? extends Enum<?>> enumClass) {
        ...
        DataDictGroup dictGroup = groupRepository.findByCode(code).orElseGet(DataDictGroup::new);
        ...
        Map<String, DataDictItem> itemMap = Optional.ofNullable(dictGroup.getItems())
                .map(Collection::stream)
                .map(it -> it.collect(Collectors.toMap(DataDictItem::getValue, Function.identity())))
        ...
        for (Enum<?> enumConstant : enumClass.getEnumConstants()) {
            ...
            // 存在即更新，否则就创建
            DataDictItem dictItem = Optional.ofNullable(itemMap.get(itemValue)).orElseGet(DataDictItem::new);
