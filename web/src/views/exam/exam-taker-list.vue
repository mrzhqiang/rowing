  ...
  <div class="app-container">
    ...
    <el-table v-loading="examLoading" :data="examList" row-key="id" size="mini"
      ...
      <el-table-column :label="$t('操作')" min-width="120" :align="'center'">
        <template v-slot="scope">
          <el-button v-permission="examPermission.takerStart"
                     size="mini" icon="el-icon-user" type="success" plain
                     @click="onExamTakerStart(scope)">{{ $t('开始考试') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import {findExam, takerExam} from '@/api/exam';
...
export default {
  ...
  methods: {
    ...
    displayPassLine(passLine) {
      return (passLine * 100.0) + '%';
    },
    findExamList() {
      this.examLoading = true;
      takerExam(this.examParams).then(response => {
    ...
    onExamTakerStart({row}) {
      // FIXME 前期为了快速开发使用 id 参数，后续为避免猜到其他考试 id，应该使用 code 参数
      this.$router.push({name: 'ExamTakerPaper', params: {id: row.id}});
    },
  }
};
</script>
